...
2. Immutable (`Hashable`) objects can be used in sets and as dictionary keys. A mutable object would be impossible to find in such a structure as its hashcode would not match its position after it's been changed. Immutability is necessary to guarantee correct behaviour of these structures.
